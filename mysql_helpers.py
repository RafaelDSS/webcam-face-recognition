...
class MySQLHelper():
    ...
    def create_mysql_table(self, table_name):
        ...
        sql = "create table if not exists " + table_name + "(milvus_id TEXT, person_name TEXT);"
        try:
    ...
    def load_data_to_mysql(self, table_name, data):
        self.test_connection()
        sql = "insert into " + table_name + " (milvus_id, person_name) values (%s,%s);"
        try:
    ...
    def search_by_milvus_id(self, id, table_name):
        self.test_connection()
        sql = "select person_name from " + table_name + " where milvus_id = " + id + " LIMIT 1;"
        try:
            self.cursor.execute(sql)
